...
class MultiThreadingF(executionContext: ExecutionContext) {
  ...
  def runMulti(toRunF1: () => Future[Unit], toRunF2: () => Future[Unit]): Future[Unit] =
    Future
      .traverse(1 to 10) { _ =>
        ...
        val f3 = Future(Thread.sleep(Random.between(1000, 3000).millis.toMillis)).flatTap(_ => Future(logger.info("sleeping")))
